...
if [ -n "${IMAGE_VER}" ] && [ -n "${SPRING_PROFILES_ACTIVE}" ] && [ -n "${FILEBEAT_OP_LS_HOSTS}" ] && [ -n "${PROJECT_MOUDLE}" ] ; then
    cd /apps/cloud-cook/swarm-prod/v3/Green
    pwd
    # 启动dop服务栈
    if [ "${PROJECT_MOUDLE}" = "INFRA" ]; then
        ...
        docker stack deploy -c cloud-cook-infra.yml --with-registry-auth cc-infra_green
    fi
    if [ "${PROJECT_MOUDLE}" = "PLATFORM" ]; then
        ...
        docker stack deploy -c cloud-cook-pfm.yml --with-registry-auth cc-pfm_green
    fi
    if [ "${PROJECT_MOUDLE}" = "APPS" ]; then
	    echo "dop apps service executing"
	    export APP_IMAGE_VER=${IMAGE_VER}
	    echo "APP_IMAGE_VER ${APP_IMAGE_VER}"
        docker stack deploy -c cloud-cook-app.yml --with-registry-auth cc-app_green
    fi
    # 查看服务列表
    docker service ls
    # 服务列表详情
